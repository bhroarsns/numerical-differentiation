...
pub fn find_root(init: f64) -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        count += 1;
        {
            let mut error = -testing(candidate) / derivative(candidate);
            while testing(candidate).abs() < testing(candidate + error).abs() {
                error /= 2.0;
            }
            candidate += error;
    ...
    let filename = format!("./newton2/{}.svg", init);
    ...
    cc.draw_series(LineSeries::new(
        vec![
            ...
            (count as i32 + 1, -0.254_461_295_051_336_86),
        ],
